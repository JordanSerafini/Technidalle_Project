...
export const getClientsQueries = (prismaService: PrismaService) => ({
  ...
  client_details: {
    ...
    questions: [
      ...
      'Peux-tu afficher la fiche signalétique de [CLIENT] ?',
      'Je veux consulter le profil de [CLIENT]',
      'Montre-moi les informations détaillées sur [CLIENT]',
      'Quel est le dossier complet de [CLIENT] ?',
      'Quels sont les contacts de [CLIENT] ?',
      'Donne-moi l\'identité de [CLIENT]',
    ],
  ...
  client_invoices: {
    ...
    questions: [
      ...
      'Quels sont les paiements attendus pour [CLIENT] ?',
      'Liste des factures ouvertes pour [CLIENT]',
      'Montre-moi les factures à venir pour [CLIENT]',
      'Quels sont les règlements en attente pour [CLIENT] ?',
      'Factures à régler pour [CLIENT]',
      'Quels sont les paiements en cours pour [CLIENT] ?',
    ],
    prisma: async (client: string) => {
  ...
  clients_with_active_projects: {
    keywords: [
      ...
      'projets en activité',
      'clients en cours de projet',
      'clients avec chantiers ouverts',
      'clients avec projets en exécution',
      'clients avec projets en développement',
      'clients avec projets en production',
    ],
    questions: [
      ...
      'Quels clients travaillent actuellement sur des projets ?',
      'Liste des clients avec des projets en activité',
      'Qui a des chantiers ouverts ?',
      'Quels clients sont en phase de réalisation de projet ?',
      'Clients avec projets en exécution',
      'Quels clients sont en production ?',
    ],
    prisma: async () => {
      return await prismaService.clients.findMany({
        where: {
  ...
  clients_by_revenue: {
    keywords: [
      ...
      'clients à haut revenu',
      'clients rentables',
      'clients à fort chiffre',
      'clients à gros budget',
    ],
    questions: [
      ...
      'Quels sont les clients à plus forte valeur ?',
      'Quels clients sont les plus rentables ?',
      'Top clients par CA',
      'Quels clients ont le plus gros budget ?',
    ],
    prisma: async () => {
  ...
  clients_without_paid_invoices: {
    keywords: [
      ...
      'clients à risque de paiement',
      'clients à surveiller',
      'clients à relancer pour impayé',
      'clients à problème de paiement',
    ],
    questions: [
      ...
      'Quels clients n\'ont jamais réglé de facture ?',
      'Liste des clients sans aucun paiement enregistré',
      'Quels clients sont à risque de non-paiement ?',
      'Quels clients sont à surveiller pour impayé ?',
      'Clients à relancer pour impayé',
      'Quels clients n\'ont jamais payé ?',
    ],
    response_format: 'table',
    prisma: async () => {
      return await prismaService.clients.findMany({
  ...
  clients_needing_follow_up: {
    keywords: [
      ...
      'clients à rappeler',
      'clients à contacter',
      'clients à relancer rapidement',
      'clients à surveiller',
      'clients à recontacter bientôt',
      'clients à suivre de près',
    ],
    questions: [
      ...
      'Quels clients nécessitent une relance rapide ?',
      'Liste des clients à rappeler',
      'Quels clients sont à contacter prochainement ?',
      'Quels clients sont à surveiller pour inactivité ?',
      'Clients à recontacter rapidement',
      'Quels clients sont à suivre de près ?',
    ],
    prisma: async () => {
      const threeMonthsAgo = new Date();
      threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);
  ...
  clients_without_ongoing_projects: {
    keywords: [
      ...
      'aucune activité',
      'sans projet actif',
      'clients sans projet en cours',
      'clients sans activité actuelle',
      'clients inactifs côté projet',
      'clients sans chantier en cours',
      'clients sans mission en cours',
      'clients sans travaux en cours',
      'clients sans projet ouvert',
      'clients sans projet démarré',
      'clients sans projet en exécution',
      'clients sans projet en développement',
      'clients sans projet en production',
    ],
    questions: [
      ...
      'Clients inactifs côté projet',
      'Quels clients n\'ont aucun projet en cours ?',
      'Montre-moi les clients sans chantier en cours',
      'Quels clients n\'ont pas de mission en cours ?',
      'Liste des clients sans travaux en cours',
      'Quels clients n\'ont pas de projet ouvert ?',
      'Quels clients n\'ont pas de projet démarré ?',
      'Quels clients n\'ont pas de projet en exécution ?',
      'Quels clients n\'ont pas de projet en développement ?',
      'Quels clients n\'ont pas de projet en production ?',
      'Montre les clients sans projet actif',
      'Quels clients sont inactifs côté projet ?',
    ],
    prisma: async () => {
      return await prismaService.clients.findMany({
        where: {
  ...
  clients_with_multiple_contacts: {
    keywords: [
      ...
      'clients avec plusieurs interlocuteurs',
      'clients multi-contacts',
      'clients avec contacts multiples',
      'clients avec plusieurs référents',
      'clients avec plusieurs points de contact',
      'clients avec contacts variés',
    ],
    questions: [
      ...
      'Quels clients ont plusieurs interlocuteurs ?',
      'Donne-moi le nombre de contacts enregistrés par client pour les clients avec plusieurs contacts.',
    ],
    prisma: async () => {
      const clients = await prismaService.clients.findMany({
  ...
  clients_with_large_invoice_volume: {
    keywords: [
      ...
      'clients à volume élevé de factures',
      'clients avec beaucoup de factures reçues',
      'clients à forte facturation',
      'clients à grand nombre de factures',
      'clients à volume de facturation important',
      'clients à facturation massive',
    ],
    questions: [
      ...
      'Quels clients ont reçu un grand nombre de factures ?',
      'Liste des clients à forte facturation',
      'Quels clients ont un volume de facturation important ?',
      'Quels clients ont une facturation massive ?',
      'Clients à grand nombre de factures',
      'Quels clients ont beaucoup de factures reçues ?',
    ],
    prisma: async () => {
      const clients = await prismaService.clients.findMany({
        include: {
  ...
  clients_with_frequent_events: {
    keywords: [
      ...
      'clients à interactions fréquentes',
      'clients à rendez-vous réguliers',
      'clients à contacts répétés',
      'clients à événements nombreux',
      'clients à forte activité événementielle',
      'clients à suivi rapproché',
    ],
    questions: [
      ...
      'Quels clients ont des interactions fréquentes ?',
      'Liste des clients à rendez-vous réguliers',
      'Quels clients ont des contacts répétés ?',
      'Quels clients ont une forte activité événementielle ?',
      'Clients à événements nombreux',
      'Quels clients nécessitent un suivi rapproché ?',
    ],
    prisma: async () => {
      const clients = await prismaService.clients.findMany({
        include: {
  ...
  clients_with_recent_activity_but_no_projects: {
    keywords: ['activité', 'contact', 'événement', 'sans projet',
      'clients actifs sans projet',
      'clients avec interactions récentes sans projet',
      'clients contactés sans chantier',
      'clients sollicités sans mission',
      'clients avec événements récents mais sans projet',
      'clients avec activité mais sans projet',
      'clients avec rendez-vous mais sans projet',
      'clients avec visites mais sans projet',
      'clients avec contacts récents mais sans projet',
    ],
    questions: [
      ...
      'Quels clients ont eu des interactions récentes sans projet ?',
      'Liste des clients sollicités sans mission',
      'Quels clients ont eu des événements récents mais aucun projet ?',
      'Quels clients ont eu des rendez-vous mais pas de projet ?',
      'Quels clients ont eu des visites mais pas de projet ?',
      'Quels clients ont eu des contacts récents mais pas de projet ?',
      'Montre-moi les clients actifs sans projet associé',
      'Quels clients sont actifs mais sans chantier ?',
    ],
    prisma: async () => {
      const threeMonthsAgo = new Date();
      threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);
  ...
  clients_with_unusual_invoice_amounts: {
    keywords: ['facture', 'anomalie', 'montant', 'élevé', 'faible',
      ...
      'factures très basses',
      'factures très élevées',
      'factures inhabituelles',
      'factures anormales',
      'factures à montant atypique',
      'factures à montant extrême',
      'factures à montant douteux',
    ],
    questions: [
      ...
      'Quels clients ont des factures exceptionnelles ?',
      'Liste des clients avec factures hors norme',
      'Quels clients ont des factures très basses ?',
      'Quels clients ont des factures très élevées ?',
      'Quels clients ont des factures à montant atypique ?',
      'Quels clients ont des factures à montant extrême ?',
      'Quels clients ont des factures à montant douteux ?',
      'Montre-moi les clients avec des factures anormales',
    ],
    prisma: async () => {
    ...
    response_format: 'table',
    description: 'Clients avec des factures anormales, très faibles ou très élevées.',
  },
  ...
  clients_with_high_discount_usage: {
    keywords: [
      ...
      'commercial',
      'clients remisés',
      'clients avec beaucoup de remises',
      'clients à forte réduction',
      'clients à remises importantes',
      'clients à remises cumulées',
      'clients à remises exceptionnelles',
      'clients à remises fréquentes',
      'clients à remises élevées',
      'clients à remises récurrentes',
      'clients bénéficiant de remises',
      'clients privilégiés',
      'clients remisés souvent',
    ],
    questions: [
      ...
      'Quels clients ont eu des remises exceptionnelles ?',
      'Quels clients ont eu des remises cumulées ?',
      'Quels clients ont eu des remises fréquentes ?',
      'Quels clients ont eu des remises élevées ?',
      'Quels clients ont eu des remises récurrentes ?',
      'Montre-moi les clients à forte réduction',
      'Quels clients ont eu beaucoup de remises ?',
      'Liste des clients bénéficiant de remises',
      'Qui sont nos clients privilégiés en termes de remises ?',
      'Clients remisés souvent',
    ],
    prisma: async () => {
      const clients = await prismaService.clients.findMany({
  ...
  clients_with_missing_contact_info: {
    keywords: [
      ...
      'clients à coordonnées incomplètes',
      'clients à informations manquantes',
      'clients à contact manquant',
      'clients à données absentes',
      'clients à informations incomplètes',
      'clients à contact incomplet',
    ],
    questions: [
      ...
      'Quels clients n\'ont pas d\'email ?',
      'Quels clients n\'ont pas de téléphone ?',
      'Quels clients n\'ont pas de coordonnées ?',
      'Quels clients ont des informations de contact manquantes ?',
      'Quels clients ont des coordonnées incomplètes ?',
      'Quels clients ont un contact manquant ?',
      'Quels clients ont des données absentes ?',
      'Quels clients ont des informations incomplètes ?',
      'Quels clients ont un contact incomplet ?',
      'Montre-moi les clients à coordonnées incomplètes',
    ],
    prisma: async () => {
      return await prismaService.clients.findMany({
        where: {
  ...
  clients_with_overdue_projects: {
    keywords: [
      ...
      'projets hors délai',
      'projets dépassant la date',
      'projets à terminer',
      'projets à finaliser',
      'projets en dépassement',
      'projets à clôturer',
      'projets en attente de clôture',
    ],
    questions: [
      ...
      'Quels clients ont des projets hors délai ?',
      'Quels clients ont des projets dépassant la date prévue ?',
      'Quels clients ont des projets à terminer ?',
      'Quels clients ont des projets à finaliser ?',
      'Quels clients ont des projets en dépassement ?',
      'Quels clients ont des projets à clôturer ?',
      'Quels clients ont des projets en attente de clôture ?',
      'Montre-moi les clients avec des projets non terminés à temps',
    ],
    prisma: async () => {
  ...
  clients_without_signed_documents: {
    keywords: [
      ...
      'contrat',
      'clients sans document signé',
      'clients sans contrat signé',
      'clients sans signature de document',
      'clients sans signature de contrat',
      'clients sans document contractuel signé',
      'clients à relancer pour signature',
      'clients avec documents en attente de signature',
      'clients sans validation de document',
      'clients sans validation de contrat',
      'clients sans engagement signé',
      'clients sans accord signé',
    ],
    questions: [
      ...
      'Clients avec documents en attente de signature',
      'Quels clients n\'ont pas de contrat signé ?',
      'Quels clients n\'ont pas validé leurs documents ?',
      'Quels clients n\'ont pas validé leur contrat ?',
      'Quels clients n\'ont pas d\'engagement signé ?',
      'Quels clients n\'ont pas d\'accord signé ?',
      'Montre-moi les clients à relancer pour signature',
      'Quels clients ont des documents en attente de signature ?',
    ],
  ...
  clients_with_upcoming_projects: {
    keywords: [
      ...
      'projets futurs',
      'projets planifiés',
      'projets clients à venir',
      'projets clients prévus',
      'projets clients futurs',
      'projets clients planifiés',
      'projets clients à démarrer',
      'projets clients à lancer',
      'projets clients à commencer',
      'projets clients à initier',
      'projets clients à programmer',
      'projets clients à planifier',
      'projets clients à organiser',
      'projets clients à préparer',
    ],
    questions: [
      ...
      'Quels clients ont des projets à venir ?',
      'Quels clients ont des projets à démarrer ?',
      'Quels clients ont des projets prévus ?',
      'Quels clients ont des projets futurs ?',
      'Quels clients ont des projets planifiés ?',
      'Quels clients ont des projets à lancer ?',
      'Quels clients ont des projets à commencer ?',
      'Quels clients ont des projets à initier ?',
      'Quels clients ont des projets à programmer ?',
      'Quels clients ont des projets à planifier ?',
      'Quels clients ont des projets à organiser ?',
      'Quels clients ont des projets à préparer ?',
      'Montre-moi les clients avec des projets à venir',
      'Montre-moi les clients avec des projets à démarrer',
      'Montre-moi les clients avec des projets prévus',
      'Montre-moi les clients avec des projets futurs',
      'Montre-moi les clients avec des projets planifiés',
    ],
    description:
  ...
  clients_without_quotes: {
    keywords: [
      ...
      'aucun devis',
      'client sans devis',
      'clients sans proposition',
      'clients sans offre',
      'clients sans devis enregistré',
      'clients sans devis reçu',
      'clients sans devis envoyé',
      'clients sans devis commercial',
      'clients sans devis commercialisé',
      'clients sans devis proposé',
      'clients sans devis transmis',
      'clients sans devis généré',
      'clients sans devis créé',
      'clients sans devis établi',
      'clients sans devis fourni',
      'clients sans devis préparé',
      'clients sans devis présenté',
      'clients sans devis soumis',
      'clients sans devis validé',
    ],
    questions: [
      'Clients sans devis',
      "Quels clients n'ont jamais reçu de devis ?",
      'Clients sans proposition commerciale',
      'Quels clients n\'ont jamais eu de devis ?',
      'Quels clients n\'ont jamais eu de proposition ?',
      'Quels clients n\'ont jamais eu d\'offre ?',
      'Quels clients n\'ont jamais eu de devis enregistré ?',
      'Quels clients n\'ont jamais eu de devis reçu ?',
      'Quels clients n\'ont jamais eu de devis envoyé ?',
      'Quels clients n\'ont jamais eu de devis commercial ?',
      'Quels clients n\'ont jamais eu de devis commercialisé ?',
      'Quels clients n\'ont jamais eu de devis proposé ?',
      'Quels clients n\'ont jamais eu de devis transmis ?',
      'Quels clients n\'ont jamais eu de devis généré ?',
      'Quels clients n\'ont jamais eu de devis créé ?',
      'Quels clients n\'ont jamais eu de devis établi ?',
      'Quels clients n\'ont jamais eu de devis fourni ?',
      'Quels clients n\'ont jamais eu de devis préparé ?',
      'Quels clients n\'ont jamais eu de devis présenté ?',
      'Quels clients n\'ont jamais eu de devis soumis ?',
      'Quels clients n\'ont jamais eu de devis validé ?',
      'Montre-moi les clients sans devis',
      'Montre-moi les clients sans proposition commerciale',
      'Montre-moi les clients sans offre',
      'Montre-moi les clients sans devis enregistré',
      'Montre-moi les clients sans devis reçu',
      'Montre-moi les clients sans devis envoyé',
      'Montre-moi les clients sans devis commercial',
      'Montre-moi les clients sans devis commercialisé',
      'Montre-moi les clients sans devis proposé',
      'Montre-moi les clients sans devis transmis',
      'Montre-moi les clients sans devis généré',
      'Montre-moi les clients sans devis créé',
      'Montre-moi les clients sans devis établi',
      'Montre-moi les clients sans devis fourni',
      'Montre-moi les clients sans devis préparé',
      'Montre-moi les clients sans devis présenté',
      'Montre-moi les clients sans devis soumis',
      'Montre-moi les clients sans devis validé',
    ],
  ...
  clients_with_projects_needing_attention: {
    ...
    questions: [
      ...
      'Quels clients ont des projets à problème ?',
      'Quels chantiers sont bloqués ?',
      'Quels clients ont des projets en alerte ?',
      'Quels projets nécessitent une attention particulière ?',
      'Quels clients ont des chantiers à risque ?',
      'Quels projets sont en souffrance ?',
      'Quels clients ont des projets bloqués ou en pause ?',
      'Quels clients ont des projets à relancer ?',
      'Quels projets sont signalés comme problématiques ?',
      'Quels clients ont des chantiers en difficulté ?',
      'Quels projets nécessitent un suivi urgent ?',
      'Quels clients sont à surveiller pour leurs projets ?',
      'Quels projets sont en situation critique ?',
      'Quels clients ont des alertes sur leurs projets ?',
      'Quels chantiers sont en attente de résolution ?',
    ],
    description:
  ...
  clients_with_high_invoice_rejections: {
    keywords: [
      ...
      'client qui rejette',
      'taux de refus élevé',
      'clients à refus fréquents',
      'clients à devis annulés',
    ],
    questions: [
      'Clients ayant refusé plusieurs devis',
      'Quels clients rejettent souvent les devis ?',
      'Clients avec beaucoup de refus commerciaux',
      'Quels clients ont un taux élevé de devis refusés ?',
      'Quels clients annulent fréquemment les devis ?',
      'Quels clients rejettent régulièrement les propositions ?',
      'Quels clients refusent souvent les factures ?',
      'Quels clients ont annulé plusieurs devis ?',
      'Quels clients sont difficiles à convaincre ?',
      'Quels clients ont un historique de refus de devis ?',
      'Quels clients ont rejeté plusieurs offres ?',
      'Quels clients sont connus pour refuser les devis ?',
      'Quels clients ont un taux de rejet commercial élevé ?',
      'Quels clients annulent ou refusent souvent les documents ?',
      'Quels clients sont à surveiller pour refus de devis ?',
      'Quels clients posent problème lors de la validation des devis ?',
    ],
    description:
  ...
  loyal_clients_by_years: {
    ...
    questions: [
      ...
      'Qui sont nos clients historiques ?',
      'Quels clients travaillent avec nous depuis le plus longtemps ?',
      'Liste des clients présents depuis plusieurs années',
      'Quels clients sont partenaires de longue date ?',
      'Quels clients ont la plus grande ancienneté ?',
      'Quels clients sont inscrits depuis le début ?',
      'Quels clients sont les plus fidèles ?',
      'Quels clients ont été créés il y a longtemps ?',
      'Quels clients sont dans notre base depuis des années ?',
      'Quels clients sont là depuis plus de 5 ans ?',
      'Quels clients ont une relation durable avec nous ?',
      'Quels clients sont présents depuis la création de l\'entreprise ?',
    ],
  ...
  clients_without_bank_info: {
    ...
    questions: [
      ...
      'Clients sans IBAN',
      'Quels clients n\'ont pas renseigné leur RIB ?',
      'Quels clients n\'ont pas d\'IBAN enregistré ?',
      'Quels clients n\'ont pas de coordonnées bancaires ?',
      'Quels clients sont sans informations bancaires ?',
      'Quels clients n\'ont pas fourni leur RIB ?',
      'Quels clients n\'ont pas de compte bancaire renseigné ?',
      'Quels clients sont à relancer pour le RIB ?',
      'Quels clients n\'ont pas de données bancaires dans leur fiche ?',
      'Quels clients n\'ont pas de moyen de paiement enregistré ?',
      'Quels clients n\'ont pas transmis leur IBAN ?',
      'Quels clients n\'ont pas de référence bancaire ?',
      'Quels clients n\'ont pas de coordonnées bancaires à jour ?',
    ],
  ...
  clients_with_no_recent_contact: {
    ...
    questions: [
      ...
      'Qui n\'a pas eu de contact depuis longtemps ?',
      'Quels clients sont sans interaction récente ?',
      'Quels clients devraient être recontactés ?',
      'Quels clients n\'ont pas eu d\'événement depuis plus de 6 mois ?',
      'Liste des clients à qui il faut redonner signe de vie',
      'Quels clients semblent avoir été oubliés ?',
      'Quels clients n\'ont pas eu de contact commercial récemment ?',
      'Quels clients sont inactifs du point de vue des interactions ?',
      'Quels clients sont listés comme jamais contactés récemment ?',
      'Quels clients sont sans trace d\'interaction depuis longtemps ?',
    ],
  ...
  clients_with_unanswered_quotes: {
    ...
    questions: [
      ...
      'Quels clients n\'ont pas répondu à leur devis ?',
      'Quels clients ont un devis en attente de réponse ?',
      'Quels clients n\'ont pas donné de retour sur leur proposition ?',
      'Quels clients ont un devis non répondu ?',
      'Quels clients sont à relancer pour la réponse au devis ?',
      'Quels clients ont des devis en attente de validation ?',
      'Quels clients n\'ont pas fait suite à leur devis récent ?',
      'Quels clients ont un devis qui traîne sans réponse ?',
      'Quels clients ont une proposition commerciale sans retour ?',
      'Quels clients doivent encore valider leur devis ?',
      'Quels clients n\'ont pas signé leur devis ?',
    ],
  ...
  projects_near_completion: {
    ...
    questions: [
      ...
      'Quels projets sont bientôt finis ?',
      'Quels chantiers sont sur le point de se terminer ?',
      'Quels projets ont un avancement supérieur à 80% ?',
      'Quels projets sont en phase finale ?',
      'Quels projets sont proches de l\'achèvement ?',
      'Quels chantiers sont en train d\'être clôturés ?',
      'Quels projets ne nécessitent plus beaucoup de travail ?',
      'Quels projets sont en fin de chantier ?',
      'Quels projets ont dépassé 80% d\'avancement ?',
      'Quels projets sont dans la dernière ligne droite ?',
      'Quels chantiers sont prêts à être livrés ?',
    ],
