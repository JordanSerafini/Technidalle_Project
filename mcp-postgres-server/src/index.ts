...
class TechnidalleMCPServer {
  ...
  private isConnected(database: string): boolean {
    switch (database) {
      case 'sync':
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (name) {
        case 'execute_query_sync':
          return this.executeQuery(args.query as string, args.limit as number, 'sync');
          
        case 'execute_query_app':
          return this.executeQuery(args.query as string, args.limit as number, 'app');
          
        case 'list_tables_sync':
          return this.listTables(args.schema as string, 'sync');
          
        case 'list_tables_app':
          return this.listTables(args.schema as string, 'app');
          
        case 'describe_table_sync':
          return this.describeTable(args.table_name as string, args.schema as string, 'sync');
          
        case 'describe_table_app':
          return this.describeTable(args.table_name as string, args.schema as string, 'app');
          
                 case 'analyze_data_sync':
           return this.analyzeData(args.table_name as string, 'sync', args.columns as string[]);
           
         case 'analyze_data_app':
           return this.analyzeData(args.table_name as string, 'app', args.columns as string[]);
  ...
  private async executeQuery(query: string, limit: number = 100, database: string): Promise<any> {
  ...
  private async listTables(schema: string = 'public', database: string): Promise<any> {
    try {
      const pool = this.getPool(database);
  ...
  private async describeTable(tableName: string, schema: string = 'public', database: string): Promise<any> {
    try {
      const pool = this.getPool(database);
      const result = await pool.query(`
        SELECT 
  ...
  private async analyzeData(tableName: string, database: string, columns?: string[]): Promise<any> {
    try {
